...
export const planYourJourney = async (req, res) => {
  ...
  try {
    ...
    if (!/^[a-zA-Z\s]+$/.test(name)) {
      res
        .status(400)
        .json({ msg: "Name must contain only letters", success: false });
    }
    if (!/^\S+@\S+\.\S+$/.test(email)) {
      res
        .status(400)
        .json({ msg: "Please enter a valid email address", success: false });
    }
    if (!/^\d{10}$/.test(phone)) {
      res
        .status(400)
        .json({ msg: "Phone number must be 10 digits long", success: false });
    }
...
export const contact=async(req,res)=>{
  ...
  try{
    ...
    if(name.length < 3){
      res.status(400).json({msg:"Name must be at least 3 characters long", success:false})
    }
    if(!/^[a-zA-Z\s]+$/.test(name)){
    ...
    if(subject.length < 3){
      res.status(400).json({msg:"Subject must be at least 3 characters long", success:false})
    }
    if(message.length < 10){
      res.status(400).json({msg:"Message must be at least 10 characters long", success:false})
    }
...
export const subscribe= async(req, res)=>{
   ...
   console.log(req.body)
    try{
      ...
      if(name.length <= 1){
        return res.status(400).json({msg:"Name must be at least 3 characters long", success:false})
      }
      if(!/^[a-zA-Z\s]+$/.test(name)){
